...
```rust
use lockstitch::{HkdfSha256Aegis128L, Protocol};

fn digest(message: &[u8]) -> [u8; 32] {
  let mut md = Protocol::<HkdfSha256Aegis128L>::new("com.example.md");
...
fn mac(key: &[u8], message: &[u8]) -> [u8; 16] {
  let mut mac = Protocol::<HkdfSha256Aegis128L>::new("com.example.mac");
  mac.mix(b"key", key);
...
fn aead_encrypt(key: &[u8], nonce: &[u8], ad: &[u8], plaintext: &[u8]) -> Vec<u8> {
  ...
  let mut aead = Protocol::<HkdfSha256Aegis128L>::new("com.example.aead");
  aead.mix(b"key", key);
