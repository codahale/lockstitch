...
use lockstitch::{subtle::ConstantTimeEq, HkdfSha256Aegis128L, Protocol, TAG_LEN};

#[test]
fn hash() {
    fn md(domain: &str, m: &[u8]) -> [u8; 32] {
        let mut md = Protocol::<HkdfSha256Aegis128L>::new(domain);
        md.mix(b"message", m);
...
fn mac() {
    fn mac(domain: &str, k: &[u8], m: &[u8]) -> [u8; TAG_LEN] {
        let mut mac = Protocol::<HkdfSha256Aegis128L>::new(domain);
        mac.mix(b"key", k);
        mac.mix(b"message", m);
...
fn stream_cipher() {
    fn enc(domain: &str, k: &[u8], n: &[u8], p: &[u8]) -> Vec<u8> {
        let mut stream = Protocol::<HkdfSha256Aegis128L>::new(domain);
        stream.mix(b"key", k);
...
fn aead() {
    fn ae_enc(domain: &str, k: &[u8], n: &[u8], d: &[u8], p: &[u8]) -> Vec<u8> {
        let mut aead = Protocol::<HkdfSha256Aegis128L>::new(domain);
        aead.mix(b"key", k);
        aead.mix(b"nonce", n);
        aead.mix(b"ad", d);
...
fn daead() {
    fn dae_enc(domain: &str, k: &[u8], d: &[u8], p: &[u8]) -> Vec<u8> {
        ...
        let mut dae = Protocol::<HkdfSha256Aegis128L>::new(domain);
        dae.mix(b"key", k);
        dae.mix(b"ad", d);
...
fn tuple_hash() {
    ...
    fn tuple_hash(domain: &str, data: &TupleVec) -> [u8; 32] {
        let mut tuple_hash = Protocol::<HkdfSha256Aegis128L>::new(domain);
        for (l, d) in data {
            tuple_hash.mix(l, d);
        }
